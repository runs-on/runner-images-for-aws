...
class DirectUefiTest < Minitest::Test
  ...
  def test_direct_cmdline_requires_portable_root_and_panic_reboot
    valid = "root=PARTUUID=abc ro panic=-1"
    ...
    refute status.success?
    assert_includes stderr, "must not write to a console"
  end

  def test_direct_cmdline_removes_bootloader_initrd_and_console_arguments
    command = "compose_direct_cmdline root=PARTUUID=abc ro console=tty1 console=ttyS0 panic=-1 BOOT_IMAGE=/vmlinuz initrd=initrd.img initrdfail"
    stdout, stderr, status = run_function(command)

    assert status.success?, stderr
    assert_equal "root=PARTUUID=abc ro panic=-1\n", stdout
  end
  ...
  def test_published_image_workflow_validates_direct_boot_state
    ...
    assert_includes workflow, %q([[ " $(< /proc/cmdline) " != *' console='* ]])
  ...
  def test_preparation_keeps_the_recovery_and_idempotency_contracts
    ...
    assert_includes script, "BOOT_IMAGE=*|initrd=*|initrdfail|initrdless_boot_fallback_triggered|console=*"
